/**
 * Popup Logic for EchoToDo
 */

document.addEventListener('DOMContentLoaded', async () => {
  console.log("EchoToDo: Popup script loaded.");
  ...
  // --- Initialization ---
  async function init() {
    // 1. Storage Check (Onboarding)
    const data = await chrome.storage.local.get(['onboardingCompleted', 'allTasks']);
    ...
    if (data.onboardingCompleted !== true) {
      ...
      // Register other listeners even if onboarding isn't done (e.g., sidepanel btn)
    ...
    if (window.SpeechModule) {
      const speechSupported = window.SpeechModule.init(
        handleSpeechResult,
        handleSpeechEnd,
        handleSpeechError
      );
    ...
    if (openSidepanelBtn) {
      openSidepanelBtn.addEventListener('click', () => {
        ...
        if (window.NotificationModule) {
          window.NotificationModule.notify("Side Panel", "Side panel ready. You can now open it from the menu.");
        }
      });
    }
  }
  ...
  function toggleRecording() {
    ...
    console.log("Toggle Recording clicked. current isRecording:", window.SpeechModule.isRecording);
    if (window.SpeechModule.isRecording) {
      window.SpeechModule.stop();
      // Note: handleSpeechEnd will handle UI cleanup
    } else {
      lastError = null; 
  ...
  function handleSpeechEnd(finalText) {
    console.log("Speech End triggered. Final text:", finalText);
    ...
    if (finalText && window.TaskProcessor) {
      ...
      if (newTasks && newTasks.length > 0) {
        ...
        saveAndRender();
        if (window.NotificationModule) {
          window.NotificationModule.notify("Tasks Captured", `Added ${newTasks.length} new tasks`);
        }
      }
    }
  ...
  function toggleTask(id) {
    ...
    if (task && task.completed && window.NotificationModule) {
      window.NotificationModule.notify("Task Completed", task.text);
    }
  }
  ...
  function renderTasks() {
    ...
    if (filtered.length === 0) {
      ...
      document.querySelectorAll('.task-item').forEach(item => {
        ...
        item.querySelector('.delete-btn').addEventListener('click', (e) => { e.stopPropagation(); deleteTask(id); });
      });
    }
  }
